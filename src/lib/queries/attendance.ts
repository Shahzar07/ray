...
import { assertCan, visibleUserIds } from "@/lib/auth/visibility";
import { can, visibilityScope } from "@/lib/domain/roles";
...
export async function getTeamAttendance(month: string, requestedTeam?: string) {
  ...
  await assertCan(ctx.user.id, teamId, "attendance.manage");
  const allowed = await visibleUserIds(ctx.user.id, teamId);
  const range = monthRange(month);
  const availableTeams =
    visibilityScope(ctx.role) === "org"
      ? await db
          .select({ id: teams.id, name: teams.name })
          .from(teams)
          .where(eq(teams.orgId, ctx.org.id))
          .orderBy(asc(teams.name))
      : ctx.teams.filter((t) => can(t.role, "attendance.manage"));
